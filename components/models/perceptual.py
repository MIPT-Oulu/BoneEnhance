...
import torch
from torch.nn import Conv3d, ReLU, MaxPool3d
        ...
        if vol:
            ...
            for x in range(4):
                ...
                if isinstance(layers_3d[x], nn.Conv3d):
                    self.slice1[x].weight[:, :, 1, :, :] = vgg_pretrained_features[x].weight
            for x in range(4, 9):
                self.slice2.add_module(str(x), layers_3d[x])
                if isinstance(layers_3d[x], nn.Conv3d):
                    self.slice2[x - 4].weight[:, :, 1, :, :] = vgg_pretrained_features[x].weight
            for x in range(9, 16):
                self.slice3.add_module(str(x), layers_3d[x])
                if isinstance(layers_3d[x], nn.Conv3d):
                    self.slice3[x - 9].weight[:, :, 1, :, :] = vgg_pretrained_features[x].weight
            for x in range(16, 23):
                self.slice4.add_module(str(x), layers_3d[x])
                if isinstance(layers_3d[x], nn.Conv3d):
                    self.slice4[x - 16].weight[:, :, 1, :, :] = vgg_pretrained_features[x].weight
        else:
            for x in range(4):
