...
from BoneEnhance.components.utilities import load, save, print_orthogonal, otsu_threshold, calculate_bvtv
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resolution_input', type=tuple, default=(200, 200, 200))  # in µm
    ...
    parser.add_argument('--completed', type=int, default=55)
    ...
    for sample in samples:
        ...
        if args.plot:
            print_orthogonal(input, savepath=str(args.th_maps / 'visualization' / (sample + '_input.png')))
        ...
        if len(np.unique(input)) != 2:
            input, _ = otsu_threshold(input)

        if args.plot:
            print_orthogonal(input, savepath=str(args.th_maps / 'visualization' / (sample + '_full_mask_input.png')))

        # Apply volume-of-interest
        input = np.logical_and(input, voi_input)
        ...
        if args.plot:
            print_orthogonal(pred, savepath=str(args.th_maps / 'visualization' / (sample + '_full_mask.png')))
            print_orthogonal(input, savepath=str(args.th_maps / 'visualization' / (sample + '_voi_mask_input.png')))
        ...
        if args.plot:
            print_orthogonal(pred, savepath=str(args.th_maps / 'visualization' / (sample + '_voi_mask.png')))
        ...
        bvtv_input = calculate_bvtv(input, voi_input)
